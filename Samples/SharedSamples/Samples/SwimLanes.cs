...
namespace Demo.Samples.SwimLanes {
  public partial class SwimLanes : DemoControl {
    ...
    public SwimLanes() {
      ...
      layoutBtn.Click += (e, obj) => _RelayoutLanes();

      desc1.MdText = DescriptionReader.Read("Samples.SwimLanes.md");
    }
    ...
    public static void RelayoutDiagram(Diagram diagram) {
                ...
                new Panel("Horizontal")  // this is hidden when the swimlane is collapsed
                  ...
                  .Add(
                    ...
                    new TextBlock { // the lane label
                        Font = new Font("Segoe UI", 13, Northwoods.Go.FontWeight.Bold, FontUnit.Point),
                        Editable = true,
            ...
            new Panel("Table") { DefaultColumnSeparatorStroke = "black" }
              .Add(
                new Panel("Horizontal") { Column = 0, Angle = 270 }
                  .Add(
                    new TextBlock {
                      Font = new Font("Segoe UI", 16, Northwoods.Go.FontWeight.Bold, FontUnit.Point),
                      Editable = true,
                      Margin = new Margin(2, 0, 0, 0)
                    }
                    .BindTwoWay("Text")
  ...
  public class LaneResizingTool : ResizingTool {
    ...
    public override Size ComputeMinSize() {
      ...
      var msz = SwimLanes.ComputeMinLaneSize(lane);  // get the absolute minimum size
      if (IsLengthening()) {  // compute the minimum length of all lanes
        var sz = SwimLanes.ComputeMinPoolSize(lane.ContainingGroup);
        msz.Width = Math.Max(msz.Width, sz.Width);
      } else { // find the minimum size of this single lane
        var sz = SwimLanes.ComputeLaneSize(lane);
        msz.Width = Math.Max(msz.Width, sz.Width);
    ...
    public override void Resize(Rect newr) {
      ...
      SwimLanes.RelayoutDiagram(Diagram);  // now that the lane has changed size, layout the pool again
    }
  }
  ...
  public class PoolLayout : GridLayout {
    ...
    public override void DoLayout(IEnumerable<Part> coll = null) {
      ...
      if (pool != null && pool.Category == "Pool") {
        ...
        var minsize = SwimLanes.ComputeMinPoolSize(pool);
        foreach (var lane in pool.MemberParts) {
          if (lane is not Group group) continue;
          if (group.Category != "Pool") {
            var shape = group.ResizeElement;
            if (shape != null) {  // change the DesiredSize to be big enough in both directions
              var sz = SwimLanes.ComputeLaneSize(group);
