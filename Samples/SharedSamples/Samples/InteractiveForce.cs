...
namespace Demo.Samples.InteractiveForce {
  public partial class InteractiveForce : DemoControl {
    ...
    public InteractiveForce() {
      ...
      desc1.MdText = DescriptionReader.Read("Samples.InteractiveForce.md");
    }

    private void Setup() {
      _Diagram.InitialAutoScale = AutoScale.Uniform;
      _Diagram.ContentAlignment = Spot.Center;
      _Diagram.Layout = new ContinuousForceDirectedLayout {
        DefaultSpringLength = 30,
        DefaultElectricalCharge = 100
      };
      // do an extra layout at the end of a move
      _Diagram.SelectionMoved += (_, e) => {
        e.Diagram.Layout.InvalidateLayout();
      };
      _Diagram.ToolManager.DraggingTool = new InteractiveForceDirectedDraggingTool();

      // define each Node's appearance
      _Diagram.NodeTemplate =
        new Node(PanelType.Auto).Add(  // the whole node panel
