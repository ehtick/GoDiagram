...
namespace Demo.Samples.Basic {
  public partial class Basic : DemoControl {
    ...
    public Basic() {
      ...
      Setup();
      desc1.MdText = DescriptionReader.Read("Samples.Basic.md");
    }
    ...
    private void Setup() {
          ...
          .Add(
            MakeButton("Properties",
              (e, obj) => {  // OBJ is this Button
                ...
                if (part is Link l) ShowDialog(LinkInfo(l.Data));
                else if (part is Group) ShowDialog(GroupInfo(contextmenu));
                else ShowDialog(NodeInfo(part.Data));
              }),
            MakeButton("Cut",
      ...
      _Diagram.GroupTemplate =
        new Group(PanelType.Vertical) {
          ...
          .Add(
            new TextBlock {
                Font = new Font("Segoe UI", 19, Northwoods.Go.FontWeight.Bold),
                IsMultiline = false,  // don't allow newlines in text
                Editable = true  // allow in-place editing by user
              }
              .Bind(
            ...
            new Panel(PanelType.Auto) { Name = "PANEL" }
              .Add(
