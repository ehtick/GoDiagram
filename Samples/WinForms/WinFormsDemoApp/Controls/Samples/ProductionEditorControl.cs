...
namespace Demo.Samples.ProductionEditor {
  [ToolboxItem(false)]
  public partial class ProductionEditorControl : DemoControl {
...
";
  ...
  ""NodeDataSource"": [
    ...
    {""Key"":10, ""Pos"":""340 210"", ""Icon"":""Finishedgas"", ""Color"":""blue"", ""Text"":""LPG, Naphtha, MTBE"", ""Description"":""Propane, butane, and other general purpose fuels and byproducts."", ""Caption"":""Liquid Petroleum Gas Truck"", ""Imgsrc"":""https://upload.wikimedia.org/wikipedia/commons/thumb/5/52/LPG_Truck.jpg/256px-LPG_Truck.jpg""},
  ...
  ""LinkDataSource"": [
    ...
    private void DefineNodeTemplates() {
      ...
      sharedNodeTemplate = new Node(PanelType.Spot) {
        ...
        new Link {
          ...
          new Shape {
            ...
            new Binding("Stroke", "FromNode", (n, _) => {
              ...
              if (((n as Node).Data as NodeData).Imgsrc != null) {
                return Brush.Lighten(colors[((n as Node).Data as NodeData).Color]);
              }
              else {
                return "gray";
              }
    ...
    private void SetupPalette() {
      ...
      (myPalette.Layout as GridLayout).CellSize = new Size(1, 1);
      myPalette.InitialContentAlignment = Spot.TopLeft;
      myPalette.InitialScale = 0.8;

      DefineNodeTemplates();
      myPalette.NodeTemplate = sharedNodeTemplate; // shared with main diagram
      myPalette.Model = new Model {
        NodeDataSource = new List<NodeData> {
          new NodeData { Icon = "Natgas", Text = "Gas Companies", Description = "Provides natural gas liquids (NGLs).", Caption = "Gas Drilling Well"},
          new NodeData { Icon = "Oil", Text = "Oil Companies", Description = "Provides associated petroleum gas (APG). This type of gas used to be released as waste from oil drilling, but is now commonly captured for processing.", Caption = "Offshore Oil Well" },
          new NodeData { Icon = "Gasprocessing", Text = "Gas Processing", Description = "APG processing turns associated petrolium gas into natural gas liquids (NGLs) and stable natural gas (SGN).", Caption = "Natural Gas Plant" },
          new NodeData { Icon = "Fractionation", Text = "Gas Fractionation", Description = "Natural gas liquids are separated into individual hydrocarbons like propane and butanes, hydrocarbon mixtures (naphtha) and liquefied petroleum gases (LPGs).", Caption = "Gas Plant" },
          new NodeData { Icon = "Pyrolysis", Text = "Pyrolysis (Cracking)", Description = "Liquefied petroleum gases (LPGs) are transformed into Ethylene, propylene, benzene, and other by-products.", Caption = "Pyrolysis Plant" },
          new NodeData { Icon = "Polymerization", Text = "Basic Polymers", Description = "Ethylene and propylene (monomers) are processed into end products using various methods (polymerization).", Caption = "Plastics engineering-Polymer products" },
          new NodeData { Text = "Intermediates", Description = "Produced Ethylene, Propylene, Butenes, Benzene, and other by-products.", Caption = " " },
          new NodeData { Icon = "Finishedgas", Text = "LPG, Naphtha, MTBE", Description = "Propane, butane, and other general purpose fuels and byproducts.", Caption = "Liquid Petroleum Gas Truck" },
        }
    ...
    private void OnSelectionChanged(object _, DiagramEvent e) {
      ...
      if (sel.Count >= 1 && sel.First() is Node node) { // a node is selected
        ...
        title.Visible = false;
        description.Text = "Select a node to see more information.";
        pictureBox1.Visible = false;
      }
    }

    private async void UpdateInformation(Node node) {
      var data = node.Data as NodeData;
      title.Visible = true;
      title.Text = data.Text;
      description.Text = data.Description;
      if (data.Imgsrc != null && data.Imgsrc != "") {
        using var response = await _HttpClient.GetAsync(data.Imgsrc);
        ...
        caption.SetToolTip(pictureBox1, data.Caption);
      } else {
        pictureBox1.Visible = false;
    ...
    private void ResizePictureBox() {
      var pb = pictureBox1;
      var imgWidth = pb.Image.Width;
      var imgHeight = pb.Image.Height;
      var ratio = (double)imgWidth / pb.Width;
      pb.Height = (int)(imgHeight / ratio);
    }
