...
namespace Demo.Samples.DecisionTree {
  public partial class DecisionTree : DemoControl {
    ...
    public DecisionTree() {
      ...
      Setup();
      desc1.MdText = DescriptionReader.Read("Samples.DecisionTree.md");
    }

    private void Setup() {
      // diagram properties
      _Diagram.InitialContentAlignment = Spot.Left;
      _Diagram.AllowSelect = false; // the user cannot select any part
      _Diagram.Layout = new TreeLayout(); // tree layout for decision tree
      ...
      void ButtonExpandCollapse(InputEvent e, GraphObject port) {
        ...
        _Diagram.ToolManager.HideToolTip();
      ...
      _Diagram.NodeTemplateMap.Add("decision",
        new Node(PanelType.Auto).Bind(
          ...
          new Panel(PanelType.Horizontal).Add(
            new TextBlock {
              Font = new Font("Segoe UI", 30, Northwoods.Go.FontWeight.Bold),
              Margin = 5
            }.Bind(
              new Binding("Text", "Key")
            ),
      ...
      _Diagram.NodeTemplateMap.Add("personality",
        new Node(PanelType.Auto).Bind(
          new Binding("Text", "Key")
        ).Add(
          new Shape("Rectangle") { Fill = "whitesmoke", Stroke = "lightgray" },
