...
namespace Demo.Samples.Navigation {
  public partial class Navigation : DemoControl {
    ...
    private string _SelectedRb = "nodesReachableRb";
    ...
    public Navigation() {
      ...
      desc1.MdText = DescriptionReader.Read("Samples.Navigation.md");
    }

    private void Setup() {
      _Diagram.MaxSelectionCount = 1; // no more than one element can be selected at a time
      ...
      _Diagram.ChangedSelection += (s, e) => _UpdateHighlights(_SelectedRb);
      _Diagram.Select(_Diagram.FindNodeForKey("A"));
    }
    ...
    private void _UpdateHighlights(string e) {
      ...
      foreach (var node in _Diagram.Nodes) node["_Highlight"] = 0;
      foreach (var link in _Diagram.Links) link["_Highlight"] = 0;

      // Get the selected GraphObject and run the appropriate method
      if (_Diagram.Selection.Count > 0) {
        var sel = _Diagram.Selection.First();

        switch (e) {
          case "linksIntoRb": _LinksTo(sel, 1); break;
          case "linksOutOfRb": _LinksFrom(sel, 1); break;
          case "linksConnectedRb": _LinksAll(sel, 1); break;
          case "nodesIntoRb": _NodesTo(sel, 1); break;
          case "nodesOutOfRb": _NodesFrom(sel, 1); break;
          case "nodesConnectedRb": _NodesConnect(sel, 1); break;
          case "nodesReachableRb": _NodesReach(sel, 1); break;
          case "containingGroupParentRb": _Containing(sel, 1); break;
          case "containingGroupsAllRb": _ContainingAll(sel, 1); break;
          case "memberNodesChildrenRb": _ChildNodes(sel, 1); break;
          case "memberNodesAllRb": _AllMemberNodes(sel, 1); break;
          case "memberLinksChildrenRb": _ChildLinks(sel, 1); break;
          case "memberLinksAllRb": _AllMemberLinks(sel, 1); break;
          case "unhighlightAllRb":
          default: break;
        }
      }
